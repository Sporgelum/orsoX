...
#files[grepl(x = files,pattern = "^[a-z]{2}[//_]{1}control_leiden.txt$")]

genesdf <- read.table(files[grepl(x = files,pattern = "^[a-z]{2}[//_]{1}control_leiden.txt$")],sep="\t", header = TRUE)
genesdf
genesdf <-genesdf[,-1]
...
dir.create(recursive = TRUE,"./control_leiden/bp")
setwd("/mnt/haus/marius/SingleCell10xKallistoJupyter/R/Enrichment/control_leiden/bp")
...
setwd("/mnt/haus/marius/SingleCell10xKallistoJupyter/R/Enrichment/control_leiden/")
dir.create("./mf")
setwd("./mf")
...
# MF  #
#######
setwd("/mnt/haus/marius/SingleCell10xKallistoJupyter/R/Enrichment/control_leiden/mf")
...
# CC  #
#######
setwd("/mnt/haus/marius/SingleCell10xKallistoJupyter/R/Enrichment/control_leiden/cc")
...
########################################
#pdf(file = "./plots.pdf")
#for (i in 1:length(mf_plots)){
#  filenames=paste0("./mf_cluster_",i,".pdf")
#  pdf(file = filenames)
#  #barplot(mf_plots[[i]],drop=T,showCategory = 25,title = paste0("cluster_",i,"_mf"))
#    print(mf_plots[i])
#  dev.off()
#}
#######################################
...
#for ( i in names(annotated_cluster_bp)) {
#  print(i)
#  pdf_names=paste0("./control_",i,"_bp_dotplot.pdf")
#  pdf(pdf_names,width = 15,height = 15)
#  print(dotplot(annotated_cluster_bp[[i]],title=paste0("control_",i,"_bp")))
#  dev.off()
#}

#pvalues <- datagenesList[,grepl("_p$",names(genesList))]
#filtered_genes <- genesList[pvalues < 0.05]
#filtered_genes

#genesList[filtered_genes]
#names(genesList)
